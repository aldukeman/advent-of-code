...
func doProblem(_ node: Node, deleteSize: Int) -> Int {
  guard node.isDirectory else { return 0 }
  let nodeSize = node.recursiveSize()
  guard nodeSize >= deleteSize else { return 0 }
  let closestChildSize = node.children
    .map { doProblem($0, deleteSize: deleteSize) }
    .filter { $0 >= deleteSize }
    .min()

  return closestChildSize ?? nodeSize
}

let root = Terminal().processInput()
print(doProblem(root, deleteSize: 30_000_000 - (70_000_000 - root.recursiveSize())))

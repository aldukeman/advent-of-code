...
let div2 = Input.Packet.list([Input.Packet.list([Input.Packet.val(2)])])
let div6 = Input.Packet.list([Input.Packet.list([Input.Packet.val(6)])])
let dividerPair = Input.MessagePair(left: div2, right: div6)

let ordered = (input.pairs + [dividerPair])
  .flatMap { [$0.left, $0.right] }
  .sorted(by: { isOrdered(lhs: $0, rhs: $1) == .less })

let mult = ordered
  .enumerated()
  .filter { $0.1 == div2 || $0.1 == div6 }
  .map { $0.0 + 1 }
  .reduce(1,*)
print(mult)

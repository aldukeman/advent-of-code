...
class Node {
  ...
  var h: Int { abs(self.coord.x - Node.end.x) + abs(self.coord.y - Node.end.y) }
...
func astar(start: Coord, end: Coord, grid: [[Int]]) -> [Coord]? {
  var frontier = [Node(g: 0, coord: start, prior: nil)]
  var visited = Set<Coord>()

  while let next = frontier.first {
    guard next.coord != end else { return next.pathFromStart() }
    ...
    let nextCoords: [Coord] = [down, up, left, right]
      ...
      .filter { grid[$0.x][$0.y] <= curHeight + 1 }
...
Node.end = input.end
let path = astar(start: input.start, end: input.end, grid: input.grid)
